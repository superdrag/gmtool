...
public class ResPack
{
   public class CodeProgress
    ...
    {
        Thread packThread = new Thread(new ParameterizedThreadStart(_PackFolder));
    ...
    {
        Thread unpackThread = new Thread(new ParameterizedThreadStart(_UnPackFolder));
        FileChangeInfo info = new FileChangeInfo();
        info.inpath = inpath;
        info.outpath = outpath;
        info.progressDelegate = progress;
    ...
    public static void PackFolder(string inpath, string outpath, ProgressDelegate progress)
    {
        ...
        _PackFolder(pathinfo);
    }

    private static void _PackFolder(object obj)
    {
        ...
        foreach (FileInfo fileinfo in dirInfo.GetFiles("*.*", SearchOption.AllDirectories))
        {
            ...
            if (fileinfo.Extension == ".meta" || fileinfo.Extension == ".upk")
            {
                continue;
            }

            Debug.Log("PackFolder0----------------- fileinfo.FullName:::" + fileinfo.FullName); //输出目录

            //规范化相对路径
            string filename = fileinfo.FullName.Replace("\\", "/");
            filename = filename.Substring(inpath.Length);
            ...
            Debug.Log("PackFolder1----------------- outpath:::" + filename); //输出目录
        ...
        for (int index = 0; index < allFileInfoDic.Count; index++)
        {
            ...
            Debug.Log("PackFolder2----------------- outpath:::" + allFileInfoDic[index].path); //输出目录
    ...
    /** 同步解包一个文件夹  upk to folder **/
    private static void _UnPackFolder(object obj)
    {
        ...
        Logger.Log("UnPackFolder:::::::::::out outpath:::" + outpath); //输出目录
        ...
        foreach (var infopair in allFileInfoDic)
        {
            ...
            // Output.Log("UnPackFolder out ------------------- path:::" + path);  //输出文件
            // Output.Log("UnPackFolder out dirpath:::" + dirpath);
            // Output.Log("UnPackFolder out filepath:::" + filepath);
...
//// =========================================  LZMA Start ==========================================
    ...
    /*
    public static void LZMACompress(string inpath, string outpath, ICodeProgress progress)
    {
    ...
    public static void LZMADeCompress(string inpath, string outpath, ICodeProgress progress)
    {
