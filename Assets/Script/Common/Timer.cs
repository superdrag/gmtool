...
public delegate void TimerCallBack(Tick tick);
...
public class Timer
{
    ...
    /// <returns></returns>
    public int AddTimer(GameObject go, float start, float interval, int count, TimerCallBack func)
    {
    ...
    /// <returns></returns>
    public int AddDeltaTimer(GameObject gameObject, float start, float interval, int count, TimerCallBack func)
    {
        if (interval < 0 || count < 0 || start < 0)
        {
            Debug.LogError("error add timer args:" + interval + count + start);
    ...
    public void CancelTimer(int tid)
    {
        ...
        {
            //Logger.Error("do delete timer no id:", tid);
        }
    }
    ...
    public void DoUpdate()
    {
        ...
        foreach (int tid in dellist)
        {
            ...
            {
                //Logger.Error("auto delete timer no id:", tid);
                continue;
            }
            deltaTickDic.Remove(tid);
        }
